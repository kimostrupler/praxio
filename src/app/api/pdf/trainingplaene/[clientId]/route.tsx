...
function TrainingsplaenePDF({ client, logoData, praxis }: { client: ClientData; logoData: string | null; praxis: PraxisConfig }) {
  return (
    <Document title={`Trainingspläne – ${client.vorname} ${client.nachname}`}>
      <Page size="A4" style={base.page}>
        <View style={base.topStrip} fixed />
        <PdfHeader title="Trainingspläne" date={formatDate(new Date())} logoData={logoData} praxisName={praxis.name} praxisSubtitle={praxis.subtitle} />
...
export async function GET(_: Request, props: { params: Promise<{ clientId: string }> }) {
  ...
  const stream = await renderToStream(<TrainingsplaenePDF client={data} logoData={logoData} praxis={praxis} />)
